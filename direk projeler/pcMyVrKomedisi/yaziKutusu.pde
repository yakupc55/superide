...
void draw(){
  if(ekranKlavyesi){
    println("ekran klavyesi yapılandırması çalışma yapısı başarılı");
    al.xKayma=30.0;
    al.yKayma=30.0;
  }
...
void aktiflikBitimi(){
  println("aktiflik bitti bro");
    ilkCizim();
    if(telefonYapilandirma){
     ekranKlavyesi=false; 
     al.xKayma=0.0; al.yKayma=0.0;
    }
}// fonksiyon sonu
...
void mouseTiklamaFonksiyonu(int x,int y){ 
  ...
  if(telefonYapilandirma){
    println("bu kısım başarılı çalışma");
  ekranKlavyesi=true;  
  }
println("x : " +x+ "y : "+y);
